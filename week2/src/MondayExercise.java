...
// Exercise 2
        ...
        for (int i = 0; i < inputString2.length(); i++) {
            for (int j = 0; j < vowelsStr.length(); j++) {
                if (inputString2.charAt(i) == vowelsStr.charAt(j)
                //題目字每一個index 都會先行一次內層loop 
                        || inputString2.charAt(i) == vowelsStr.toUpperCase().charAt(j)) {
                    System.out.print(inputString2.charAt(i) + " ");
                }
            }
        }
        ...
        //using indexof : (find the first occur index, can use)
