...
public class DemoUncheckedException {
    ...
    public static void main(String[] args) {
        ...
        try {
            concat(null, "abc");  //消化這炸彈   //java.lang.illegalAtgu 
        } catch (IllegalArgumentException e) {//can empty
...
//throw ->Exceltion class object
    ...
    public static String concat(String s1, String s2) {       //Argument = Parameter
        if (s1 == null) {
            throw new IllegalArgumentException(); //IAE  //throw = 出error的return
        }
        return s1.concat(s2);
    }
...
//throw Business Exception
    public static int calculate(int x, int y) {
        if (x < 0 || y < 0) {
            throw new BusinessRuntimeException();   //throwed still error
